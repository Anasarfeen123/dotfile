...
hl.bind("CTRL + ALT + Delete", hl.dsp.global("quickshell:sessionToggle"), { description = "Session: Power Menu" })
hl.bind("SUPER + Escape", hl.dsp.global("quickshell:sessionToggle"), { description = "Session: Power Menu" })
...
hl.bind("SUPER + SHIFT + T", hl.dsp.exec_cmd(
    qsIsAlive ..
    " || pidof slurp || grim -g \"$(slurp $SLURP_ARGS)\" \"/tmp/ocr_image.png\" && LANG_LIST=\"$(tesseract --list-langs 2>/dev/null | awk 'NR>1 && $1!=\"osd\"{print $1}' | sort -u | paste -sd+)\"; [ -z \"$LANG_LIST\" ] && LANG_LIST=\"eng\"; tesseract \"/tmp/ocr_image.png\" stdout -l \"$LANG_LIST\" | wl-copy && rm \"/tmp/ocr_image.png\""
))
--# Color picker
...
local superScrollConfigPath = HOME .. "/.config/illogical-impulse/config.json"
local reverseSuperScroll = false
local configFile = io.open(superScrollConfigPath, "r")
if configFile then
    local configContent = configFile:read("*a")
    configFile:close()
    if configContent:find('"reverseSuperScroll"%s*:%s*true') then
        reverseSuperScroll = true
    end
end
for i = 1, 4 do
    local key = { "SUPER + mouse_up", "SUPER + mouse_down" }
    local keycombos = { key[1], key[2], "CTRL + " .. key[1], "CTRL + " .. key[2] }
    local prefix = reverseSuperScroll and { "-", "+", "r-", "r+" } or { "+", "-", "r+", "r-" }
    hl.bind(keycombos[i], hl.dsp.focus({ workspace = prefix[i] .. "1" }))
end
--## Special
...
hl.bind("SUPER + SHIFT + E", hl.dsp.exec_cmd(hyprScripts .. "/glass-off.sh"),
    { description = "Glass: Disable transparency (opaque)" })
hl.bind("SUPER + SHIFT + D", hl.dsp.exec_cmd(hyprScripts .. "/glass-on.sh"),
    { description = "Glass: Restore transparency" })
