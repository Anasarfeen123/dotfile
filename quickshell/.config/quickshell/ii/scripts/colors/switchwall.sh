...
sync_sddm_theme() {
    # Copies the matugen-rendered SDDM theme to the system theme dir.
    # Relies on a scoped NOPASSWD sudoers rule for exactly this command
    # (/etc/sudoers.d/ii-sddm-matugen); non-fatal if that rule isn't present.
    [ -f /tmp/sddm-matugen.conf ] || return 0
    sudo -n cp /tmp/sddm-matugen.conf /usr/share/sddm/themes/silent/configs/matugen.conf 2>/dev/null || true
}

apply_amoled_override() {
    # Opt-in post-processing pass: forces the base canvas (background/surface)
    # to true black across the already-generated theme files, leaving elevated
    # "container" surfaces as matugen generated them (preserves M3 hierarchy).
    # Runs BEFORE sync_sddm_theme so it only ever touches local /tmp/state files -
    # the sudoers rule is scoped to one exact `cp` of /tmp/sddm-matugen.conf, so
    # this must never need sudo itself.
    if [ ! -f "$SHELL_CONFIG_FILE" ]; then
        return 0
    fi
    local amoled
    amoled=$(jq -r '.appearance.amoled' "$SHELL_CONFIG_FILE")
    if [ "$amoled" != "true" ]; then
        return 0
    fi

    # Quickshell's material colors (background, surface, surface_dim -> #000000)
    local colors_json="$STATE_DIR/user/generated/colors.json"
    if [ -f "$colors_json" ]; then
        jq '.background = "#000000" | .surface = "#000000" | .surface_dim = "#000000"' \
            "$colors_json" > "$colors_json.tmp" && mv "$colors_json.tmp" "$colors_json"
    fi

    # Hyprlock's $surface_color (keep its existing alpha suffix, blacken the RGB)
    local hyprlock_colors="$XDG_CONFIG_HOME/hypr/hyprlock/colors.conf"
    if [ -f "$hyprlock_colors" ]; then
        sed -i -E 's/(\$surface_color = rgba\()[0-9a-fA-F]{6}([0-9a-fA-F]{2}\))/\1000000\2/' "$hyprlock_colors"
    fi

    # SDDM's LockScreen/LoginScreen background-color, patched in the LOCAL
    # /tmp/sddm-matugen.conf (section-aware; leaves elevated surface_container_*
    # backgrounds on password/login buttons etc. alone). sync_sddm_theme (called
    # right after this) then ships this exact file to the system theme dir.
    if [ -f /tmp/sddm-matugen.conf ]; then
        python3 - << 'PYEOF'
import re
path = "/tmp/sddm-matugen.conf"
with open(path) as f:
...
for line in lines:
    ...
    if m:
        ...
        out.append(line)
        continue
    if section in ("LockScreen", "LoginScreen") and line.strip().startswith("background-color"):
        out.append('background-color = "#000000"')
    else:
        out.append(line)
