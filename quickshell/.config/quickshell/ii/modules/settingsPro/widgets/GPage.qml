...
import qs.modules.common.widgets as CW // reused only for scroll physics (StyledFlickable/StyledScrollBar
                                        // bind to the user's own Config.options.interactions.scrolling
                                        // settings) — not part of the visual widget skin.

// Scrollable page host with section-tracking, so the nav rail's "on this page" list and
// the app's search-and-scroll both work exactly like the original ContentPage.
CW.StyledFlickable {
    id: root
    property real baseWidth: 640
