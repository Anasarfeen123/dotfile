    ...
    function getCommand(x, y, width, height, screenshotPath, action, saveDir = "") {
        ...
        switch (action) {
            ...
            case ScreenshotAction.Action.CharRecognition:
                return ["bash", "-c", `${cropInPlace} && LANG_LIST="$(tesseract --list-langs 2>/dev/null | awk 'NR>1 && $1!="osd"{print $1}' | sort -u | paste -sd+)"; [ -z "$LANG_LIST" ] && LANG_LIST="eng"; tesseract ${StringUtils.shellSingleQuoteEscape(screenshotPath)} stdout -l "$LANG_LIST" | wl-copy && ${cleanup}`]
                break;
