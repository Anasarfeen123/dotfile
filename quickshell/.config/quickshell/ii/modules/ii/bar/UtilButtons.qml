...
Item {
    ...
    RowLayout {
        ...
        Loader {
            active: Config.options.bar.utilButtons.showAwakeToggle
            visible: Config.options.bar.utilButtons.showAwakeToggle
            sourceComponent: CircleUtilButton {
                Layout.alignment: Qt.AlignVCenter
                toggled: Idle.inhibit
                onClicked: Idle.toggleInhibit()
                content: Item {
                    implicitWidth: Math.max(26, awakeIcon.implicitWidth + 8)
                    implicitHeight: 26
                    MaterialSymbol {
                        id: awakeIcon
                        anchors.centerIn: parent
                        ...
                        text: "coffee"
                        iconSize: Appearance.font.pixelSize.large
                        color: Idle.inhibit ? Appearance.m3colors.m3onPrimary : Appearance.colors.colOnLayer2
                    }
                    StyledToolTip {
                        text: Idle.inhibit ? Translation.tr("Keep system awake") : Translation.tr("Allow system to sleep")
                    }
                }
            }
        }
