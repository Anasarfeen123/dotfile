...
Item {
    ...
    WheelHandler {
        onWheel: (event) => {
            const reversed = GlobalStates.superDown && (Config?.options.interactions.scrolling.reverseSuperScroll ?? false);
            if (event.angleDelta.y < 0)
                Hyprland.dispatch(`hl.dsp.focus({workspace = "r${reversed ? "-" : "+"}1"})`);
