...
Singleton {
    ...
    function searchFiles() {
        const searchString = StringUtils.cleanPrefix(root.query, Config.options.search.prefix.file).trim();
        ...
        fileProc.command = ["fd", "--fixed-strings", "--type", "f", "--max-results", "40", searchString, FileUtils.trimFileProtocol(Directories.home)];
    ...
    onQueryChanged: {
        if (root.query.startsWith(Config.options.search.prefix.file)) {
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: (line) => {
                fileProc.buffer.push(line)
            }
        }
        onExited: (exitCode, exitStatus) => {
            if (exitCode === 0) {
                root.fileResults = fileProc.buffer.map(path => {
    ...
    property list<var> results: {
        ...
        } else if (root.query.startsWith(Config.options.search.prefix.windows)) {
            // Switch to a running window
            const searchString = StringUtils.cleanPrefix(root.query, Config.options.search.prefix.windows).trim().toLowerCase();
            return HyprlandData.windowList
                .filter(window => {
                    if (!searchString) return true;
                    const haystack = `${window.title} ${window.class} ${window.initialClass} ${window.initialTitle}`.toLowerCase();
                    return haystack.includes(searchString);
                })
                .sort((a, b) => {
                    const aPinned = a.workspace.id === HyprlandData.activeWorkspace?.id ? 0 : 1;
                    const bPinned = b.workspace.id === HyprlandData.activeWorkspace?.id ? 0 : 1;
                    return aPinned - bPinned;
                })
                .slice(0, 20)
                .map(window => {
                    const entry = DesktopEntries.heuristicLookup(window.class || window.initialClass);
                    const icon = entry?.icon || "";
                    return resultComp.createObject(null, {
                        name: window.title || window.class || window.address,
                        type: Translation.tr("Window"),
                        verb: Translation.tr("Focus"),
                        iconName: icon !== "" ? icon : "open_in_new",
                        iconType: icon !== "" ? LauncherSearchResult.IconType.System : LauncherSearchResult.IconType.Material,
                        execute: () => {
        ...
        } else if (root.query.startsWith(Config.options.search.prefix.weather)) {
            // Weather for a city
            const searchString = StringUtils.cleanPrefix(root.query, Config.options.search.prefix.weather).trim();
            return [resultComp.createObject(null, {
                name: searchString ? Translation.tr("Get weather for %1").arg(searchString) : Translation.tr("Get weather"),
                type: Translation.tr("Weather"),
                verb: Translation.tr("Search"),
                iconName: "partly_cloudy_day",
                iconType: LauncherSearchResult.IconType.Material,
                execute: () => {
                    Qt.openUrlExternally(`https://wttr.in/${encodeURIComponent(searchString.replace(/\s+/g, '+') || Config.options.bar.weather.city)}`);
                }
            })];
        } else if (root.query.startsWith(Config.options.search.prefix.file)) {
            // File search (results arrive async via fileProc)
